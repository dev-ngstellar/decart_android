...
const checkPayload = async (noTelefon) => {
  ...
  const Forgotpassword = async()=>{  
    ...
    if (response && response.payload.CustID != 0) {
      resetForm()
   navigation.navigate("Validation",{payload})
    } else{
      resetForm()
...
// const getOtp =async(CustId)=>{
//   const payload={
//     CustId:CustId
//   }

//  const getOtpRes= await dispatch(GenerateOTPThunk({payload}))

//  if(getOtpRes && getOtpRes.payload.APIResult==='SUCCESS'){
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={{height: 100,width:'95%',marginHorizontal:'3%'}}>
        <Text style={{fontSize: 24, fontWeight: 'bold',color:textColor}}>Terlupa Kata Laluan?</Text>
        <Text style={{fontSize:16,color:textColor}}>Masukkan no telefon atau e-mel dan kami akan menghantar kod
